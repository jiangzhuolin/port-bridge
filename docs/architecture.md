...
Flutter's standard Windows, macOS and Linux runners host the UI. Builds run on matching OS/CPU hosts, and artifact names include `x86_64` or `arm64`. `tool/build.dart` refuses a mismatched requested architecture. macOS builds explicitly set the Xcode architecture and verify the main executable with `lipo`. Linux packaging verifies all ELF architectures and creates DEB/RPM packages with system runtime dependency metadata, alongside the portable tar.gz bundle. macOS also produces a verified DMG. See [package details](packages.md).
